...
Page({
  ...
  data: {
    option1: [
      { text: '待派单', value: 'A' },//01
      { text: '已派单', value: 'E' },//03,05
      // { text: '已派单', value: 'B' },//03,05
    ],
    multProcStat: 'A',
    repairLogList:[],
    pageNum:1,
  ...
  queryForPage(type){
    ...
    netWork.RequestMQ.request({
      url:api.queryPaiDan,
      method:'POST',
      data:{custId:custId,pageNum:pageNum,pageSize:pageSize,multProcStat:multProcStat},
      success:res=>{
        that.showLoading(0);
        var datas = res.data;
  ...
  onLoad: function (options) {
    ...
    if(assign == 'P'){
      Toast.alert({message:'当前是接单模式，无法进行派单业务'}).then(()=>{
        // wx.navigateTo({
        //   url: '../orders/orders',
        // })
      })
