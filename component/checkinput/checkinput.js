...
Component({
  ...
  properties: {
    ...
    value_length: {
      ...
      observer: function (newVal, oldVal) {
        ...
        // 当输入框的值等于6时（发起支付等...）
        that.triggerEvent('finishInput', input_value)
        // if (newVal == 6) {
        //   // 设定延时事件处理
        //   setTimeout(function () {
  ...
  methods: {
    ...
    blur() {
      ...
      that.triggerEvent('inputBlur')
    },

    // 点击聚焦
    set_focus() {
      let that = this;
      that.triggerEvent('inputFocus')
      that.setData({
        get_focus: true
      })
    },
    ...
    get_value(data) {
      ...
      // 获取输入框值的长度
      let value_length = now_val.length;
      // 更新数据
      that.setData({
